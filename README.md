...
> $506.11 per patient, compared with $761.94 for standalone office biopsy and $3,839.81 for
> operative dilation and curettage. Adding EMB to colonoscopy cost $255.83 less than performing EMB
> separately (33.6%). The combined strategy remains the least expensive strategy as long as the
> incremental colonoscopy-suite time stays below approximately 12.7 minutes.
...
the attempt studied -- independently confirmed as the exact subgroup Yi et al. 2018 cite for the
identical purpose in their own methods text). A repeat attempt costs the same $255.93 as the initial
visit, since it bills as an established-patient encounter.
...
(`repeat_attempt_fraction x repeat_attempt_success_probability`) is under 4% even at the upper 95% CI
...
expensive strategy in 90.5% of draws; D&C was never the least expensive strategy in any draw.*
...
with its advantage over office EMB widening from $207.12 (low-cost) to $348.75 (high-cost) per patient
...
(the 90.5% probability combined EMB is cheaper, the 0.35-vs-2.51-per-1,000 adverse-event exposure,
