...
> $553.45 per patient, compared with $914.38 for standalone office biopsy and $4,387.14 for
> operative dilation and curettage. Adding EMB to colonoscopy cost $360.92 less than performing EMB
> separately (39.5%). The combined strategy remains the least expensive strategy as long as the
> incremental colonoscopy-suite time stays below approximately 15.8 minutes.
...
Every parameter in `config/model_parameters.csv` has a `provisional` column. As of this writing, 10
of 39 parameters are flagged `TRUE`, including several that materially affect the base-case
result: the D&C preop-visit, recovery-room, and anesthesia costs (the D&C *facility* fee, formerly
the largest provisional input in the model, is now a real, sourced CMS OPPS rate -- see
