...
> $530.37 per patient, compared with $816.40 for standalone office biopsy and $3,827.04 for
> operative dilation and curettage. Adding EMB to colonoscopy cost $286.03 less than performing EMB
> separately (35.0%). The combined strategy remains the least expensive strategy as long as the
> incremental colonoscopy-suite time stays below approximately 13.6 minutes.
...
of 40 parameters are flagged `TRUE`: `emb_disposable_supply_cost`; the coordination-time half of
`coordination_cost` (its wage half is now a real BLS/O*NET figure); `office_to_dnc_escalation_fraction`
(the assumption that 100% of failed office EMB attempts escalate to D&C); `combined_requires_preop_office_visit`
(a structural scenario toggle); the CPT 58558 data-quality-flagged `hysteroscopy_dc_professional_cost`
(not used in the base case); and two `reference_only` values kept only as documented exclusions
...
office-EMB arm's E/M visit cost. `docs/data_sources.md` lists what each remaining item still needs.
