...
> $540.26 per patient, compared with $875.26 for standalone office biopsy and $4,101.64 for
> operative dilation and curettage. Adding EMB to colonoscopy cost $335.01 less than performing EMB
> separately (38.3%). The combined strategy remains the least expensive strategy as long as the
> incremental colonoscopy-suite time stays below approximately 15.0 minutes.
...
Every parameter in `config/model_parameters.csv` has a `provisional` column. As of this writing, 9
of 40 parameters are flagged `TRUE`, including several that materially affect the base-case
result: the D&C preop-visit and recovery-room costs (the D&C *facility fee* and *anesthesia cost*,
formerly the two largest provisional inputs in the model, are now real, sourced CMS values -- see
`docs/data_sources.md`); the office-EMB disposable-supply and E/M visit costs; the coordination cost
for the combined arm (its wage component is now a real BLS/O*NET figure, but the coordination-time
estimate is the PI's own practitioner judgment, not a published source); and the assumption that
100% of failed office EMB attempts escalate to D&C. `docs/data_sources.md` lists what each one still
needs. **No provisional value should be quoted as a finding.**
