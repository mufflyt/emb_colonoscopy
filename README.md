...
> $543.18 per patient, compared with $875.26 for standalone office biopsy and $4,101.64 for
> operative dilation and curettage. Adding EMB to colonoscopy cost $332.09 less than performing EMB
> separately (37.9%). The combined strategy remains the least expensive strategy as long as the
> incremental colonoscopy-suite time stays below approximately 14.9 minutes.
...
of 39 parameters are flagged `TRUE`, including several that materially affect the base-case
...
for the combined arm; and the assumption that 100% of failed office EMB attempts escalate to D&C.
`docs/data_sources.md` lists what each one still needs. **No provisional value should be quoted as a
finding.**
