...
| 26 | Study findings, limitations, generalizability, and current knowledge | Reported | Discussion | Full limitations discussion including the (as of 2026-09-02, now symmetric) absence of an estimable delayed-neoplasia risk for either arm, the office repeat-attempt structure's small non-Lynch evidence base, D&C's own unmodeled failure risk, partially-monetized adverse-event costs, and non-Lynch-specific parameter sources, each paired with what evidence would resolve it. **Flagged as a first-pass author draft pending final clinical/policy review, not yet author-confirmed** -- see the DRAFT comment at the top of that section in `manuscript.qmd`. |
