...
| `compute_colonoscopy_opportunity_cost_bound matches an independent from-scratch recomputation` and 2 other tests in `test-opportunity-cost-colonoscopy.R` (Mississippi's negative-margin finding, base-case non-consumption guard held) | In `compute_colonoscopy_opportunity_cost_bound()` (`R/opportunity_cost_colonoscopy.R`), flipped `commercial_mean_rate - medicare_rate` to `commercial_mean_rate + medicare_rate` | RED with defect on 3 tests simultaneously (Denver Health's `commercial_margin` off by exactly `2 x medicare_rate = 1962`; the derived `opportunity_cost_ceiling` off by the corresponding scaled amount; and, most tellingly, Mississippi's real negative-margin finding flipped to a large positive `1489.4`, since adding two positive rates can never be negative -- a live demonstration that this specific mutation would have silently erased the one hospital where the direction of the finding matters most) -> GREEN on revert |
